    ...
    <el-card id="resultTable">
      <div slot="header" class="clearfix">
        <el-row type="flex" justify="space-between">
          ...
          <el-col :span="2" class="no-print">
            <el-button type="warning" icon="el-icon-printer" plain class="ext-button" @click="$emit('print', {title: '运营商报告'})">打印</el-button>
          </el-col>
        </el-row>
      </div>
      <table class="table card-text">
        <tr class="text-left">
          <td width="20%">查询结果</td>
          <td width="30%">
            <el-tag :type="result.code | resultLogoFmt" class="tag">{{result.code | resultSuccessFmt}}</el-tag>
          </td>
          <td width="20%">报告编号</td>
        ...
        <tr class="inner-table">
          <td colspan="4" style="padding: 0;">
            <table class="table">
              ...
              <tr v-if="result.data.overdueLoanCheck.length == 0">
        ...
        <tr class="inner-table">
          <td colspan="4" style="padding: 0;">
            <table class="table">
              ...
              <tr v-if="result.data.multiLendCheck.length == 0">
                <td colspan=4 class="centerAlign">无数据</td>
              </tr>
        ...
        <tr class="inner-table">
          <td colspan="4" style="padding: 0;">
            <table class="table">
              <tr>
                <th rowspan=2>检查项</th>
                <th rowspan=2>命中描述</th>
                <th rowspan=2>命中次数</th>
                <th rowspan=2>时长(s)</th>
                <th colspan=4>详情</th>
              </tr>
              <tr>
                <th>通话标记</th>
              ...
              <tr v-if="result.data.riskCallCheck.length == 0">
                <td colspan=4 class="centerAlign">无数据</td>
              </tr>
        ...
        <template v-if="result.data.scoreAnalysis">
          ...
          <tr>
            <td colspan="1">最后得分</td>
            <td colspan="3">{{result.data.scoreAnalysis.score}}</td>
          </tr>
        </template>
      </table>
    </el-card>
...
export default {
    ...
    data () {
        ...
        const operatorValueFmt = {
            isHitRiskList: function (v) {
                return v == "1"
                    ? '<span class="tag el-tag el-tag--error">命中</span>'
                    : '<span class="tag el-tag el-tag--info">未命中</span>';
            },
        ...
        return {
            ...
            rules: {
                ...
                contactMobile1st: [
                    { validator: linkManPhoneValidator, trigger: "blur" }
                ],
                contactMobile2nd: [
                    { validator: linkManPhoneValidator, trigger: "blur" }
                ],
                ...
                contactIdentityNo1st: [
                    { validator: linkManIdValidator, trigger: "blur" }
                ],
                contactIdentityNo2nd: [
                    { validator: linkManIdValidator, trigger: "blur" }
                ],
                contactIdentityNo3rd: [
                    { validator: linkManIdValidator, trigger: "blur" }
                ]
            },
            result: {
    ...
    methods: {
                ...
                if (valid) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        if (vm.linkMan.includes(i + 1 + "")) {
                            if (
                                !(
                                    vm.inputForm["contactName" + engNumArr[i]] &&
                                    vm.inputForm["contactMobile" + engNumArr[i]] &&
                                    vm.inputForm["contactIdentityNo" + engNumArr[i]] &&
                                    vm.inputForm["contactRelationship" + engNumArr[i]]
                                )
                            ) {
                                vm.$message({
                                    showClose: true,
                                ...
                                params["contactName" + engNumArr[i]] =
                                    vm.inputForm["contactName" + engNumArr[i]];
                                params["contactMobile" + engNumArr[i]] =
                                    vm.inputForm["contactMobile" + engNumArr[i]];
                                params["contactIdentityNo" + engNumArr[i]] =
                                    vm.inputForm["contactIdentityNo" + engNumArr[i]];
                                params["contactRelationship" + engNumArr[i]] =
                                    vm.inputForm["contactRelationship" + engNumArr[i]];
                            }
                        }
                    }
        ...
        startTask: function (params) {
            ...
            vm.loading = true;
            vm.$http
                .get("api/rip/operatorCreditReports/result", {
                    params: params,
                    headers: {
        ...
        getStatus: function () {
            ...
            vm.$http
                .get("api/rip/operatorCreditReports/status", {
                ...
                .then(function (res) {
                    if (res.data.code == "0006") {
                        ...
                        ["0000", "0100", "0200"].includes(res.data.code)
                    ) {
                        vm.tryAgain = 10;
                        vm.$message({
                            showClose: true,
                            message: "正在获取报告,请耐心等待",
                            type: "info"
                        });
                        vm.loading &&
                            (vm.timeout = setTimeout(vm.getResult, 5000));
                    } else {
                        vm.tryAgain--;
                        if (res.data.msg) {
                            vm.$message({
                        ...
                        vm.loading &&
                            (vm.timeout = setTimeout(vm.getStatus, 5000));
                    }
                });
        },
        inputValidateCode: function (value) {
            var vm = this;
            vm.$http
                .get("api/rip/operatorCreditReports/input", {
        ...
        getResult: function () {
            ...
            vm.tryAgain--;
            vm.$http
                .get("api/rip/operatorCreditReports/report", {
                ...
                .then(function (res) {
                    if (res.data.code == "0000") {
                        ...
                        vm.loading &&
                            (vm.timeout = setTimeout(vm.getResult, 3000));
                    }
                });
        },
        open () {
